...
- [ ] `make demo-interop` target exists in the Makefile at `native-schema-registry/golang/Makefile`.
...
- [ ] `make -n demo-interop` (dry run) succeeds without errors from the Makefile at `native-schema-registry/golang/`.
...
```bash
cd native-schema-registry/golang && make -n demo-interop
```
