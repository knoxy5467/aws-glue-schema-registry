...
```
multilang-schema-registry/golang/integration-tests/
├── cmd/
│   └── demo-interop/
...
5. **Java sidecar JAR built:** `make java-sidecar-build` (from `multilang-schema-registry/golang/`).
6. **`default-registry` exists** in Glue in `us-east-2` (see `REAL-AWS-RUNBOOK.md`).

### 8.2 Running the Demo

From `multilang-schema-registry/golang/`:
