 ...
 *
 * Drives the FULL Kafka customer API — exact same library entry point a
 * real Java GSR Kafka consumer would use.
 *
 * Request body (JSON):
 *   {
 ...
 *      envelope the caller asserts against.
 *
 * Response body (JSON):
...
public final class KafkaConsumeHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            // Wrapped in try-with-resources because GlueSchemaRegistryKafkaDeserializer
            // implements Closeable and holds internal Glue client resources.
            Object javaRecord;
            try (GlueSchemaRegistryKafkaDeserializer kafkaDeserializer =
                         new GlueSchemaRegistryKafkaDeserializer(gsrConfigs)) {
                javaRecord = kafkaDeserializer.deserialize(topic, framed);
            }

            // 3. Resolve schema metadata for the response (so the caller
