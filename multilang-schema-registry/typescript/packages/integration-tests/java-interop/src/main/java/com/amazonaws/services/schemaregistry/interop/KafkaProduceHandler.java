 ...
 *
 * Drives the FULL Kafka customer API — same library entry point a real
 * Java GSR Kafka producer would use — so the test exercises the format-
 * specific serializer (including PROTOBUF's message-index prefix) and the
 * Glue register/lookup, not just the wire-format envelope.
 *
 * Request body (JSON):
 *   {
