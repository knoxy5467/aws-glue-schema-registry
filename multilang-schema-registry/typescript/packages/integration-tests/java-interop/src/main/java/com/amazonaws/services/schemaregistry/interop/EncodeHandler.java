 ...
 * Java object): the wire-format endpoint's contract is the wire-format
 * layer, NOT format-specific serialization. The caller side already does
 * the format encoding; the sidecar's job is to prove the framing layer is
 * byte-for-byte identical across languages. Adding a JSON-or-Avro-or-
 * Protobuf record serializer here would force a second round of
 * cross-language object-shape negotiation, which is what the Kafka-in-the-
 * loop endpoints (/kafka-produce, /kafka-consume) are for.
 */
